function master_rsa_dh_aes_demo_fixed()
% MASTER_RSA_DH_AES_DEMO_FIXED
% Full demo (fixed): Diffie-Hellman key exchange (with channel simulation)
% -> derive AES-256 key (SHA-256) -> AES-CBC encrypt PDF -> RSA sign SHA-512 hash
% -> transmit (simulated channel) encrypted file + signature -> receiver decrypts
% and verifies signature.
%
% Notes:
% - Uses Java BigInteger and Java crypto via MATLAB.
% - For demo speed, uses 512-bit primes. Use 2048+ in production.
% - Channel error probabilities default to 0 (ideal). Set >0 to demonstrate corruption.

clc; clear; close all;
fprintf('====================================================================\n');
fprintf(' MASTER DEMO (FIXED): Diffie-Hellman + AES-CBC (file) + RSA signature\n');
fprintf('====================================================================\n\n');
...
channelErrorProb_file       = 0.0;   % for encrypted file transfer
channelErrorProb_signature  = 0.0;   % for signature transfer
...
fprintf('STEP SIG-1: SHA-512 hash of original file (hex):\n%s\n\n', hash_hex(1:min(200,numel(hash_hex))));
...
try
    ...
    error('Signature transmission corrupted into non-hex ASCII. Reduce channel error prob.');
end
...
fprintf('\n====================================================================\n');
fprintf(' END OF MASTER DEMO (FIXED)\n');
fprintf('====================================================================\n\n');
